...
class SlidingTabStrip extends LinearLayout {
    ...
    SlidingTabStrip(Context context, AttributeSet attrs) {
        ...
        mBottomBorderThickness =
                (int)(DEFAULT_BOTTOM_BORDER_THICKNESS_DIPS * density);
        mBottomBorderPaint = new Paint();
        mBottomBorderPaint.setColor(defaultBottomBorderColor);

        mSelectedIndicatorThickness =
                (int)(SELECTED_INDICATOR_THICKNESS_DIPS * density);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final SlidingTabLayout.TabColorizer tabColorizer =
                mCustomTabColorizer != null ? mCustomTabColorizer
    ...
    private static int blendColors(int color1, int color2, float ratio) {
        ...
        float g =
                (Color.green(color1) * ratio) + (Color.green(color2) * inverseRation);
        float b =
                (Color.blue(color1) * ratio) + (Color.blue(color2) * inverseRation);
    ...
    private static class SimpleTabColorizer
        ...
        void setIndicatorColors(int... colors) { mIndicatorColors = colors; }
    }
}
